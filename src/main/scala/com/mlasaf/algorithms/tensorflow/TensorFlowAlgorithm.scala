package com.mlasaf.algorithms.tensorflow
...
class TensorFlowAlgorithm extends AlgorithmInstance {

  /** */
  def onAlgorithmRun(run : AlgorithmRun) : AlgorithmExitParams = {
    println(" ===================== RUNNING TENSORFLOW ALGORITHM FOR RUN: " + run.algorithmRunDto.algorithmRunId + ", input views: " + run.executorStorageViewDtos.size + "" + run.outputs.size);
