package com.mlasaf.algorithms.r
...
class RScriptAlgorithm extends AlgorithmInstance {

  /** */
  def onAlgorithmRun(run : AlgorithmRun) : AlgorithmExitParams = {
    println(" ===================== RUNNING R ALGORITHM FOR RUN: " + run.algorithmRunDto.algorithmRunId + ", input views: " + run.executorStorageViewDtos.size + ", outputs.size: " + run.outputs.size);
